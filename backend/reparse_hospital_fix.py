...
c = conn.cursor()
c.execute("SELECT report_id, file_name FROM Report WHERE is_delete=0 AND (hospital_id IS NULL OR hospital_id=5)")
reports = c.fetchall()
conn.close()

print(f"需要重新解析的报告数（医院未识别或错误）: {len(reports)}")
print("=" * 80)
...
for idx, (report_id, file_name) in enumerate(reports):
    try:
        print(f"\n[{idx+1}/{len(reports)}] 重新解析: {file_name}")
