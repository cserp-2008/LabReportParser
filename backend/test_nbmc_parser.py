...
for fname in sorted(os.listdir(pdf_dir)):
    ...
    for p in pages:
        ...
        if p.ocr_coords_text:
            full_text += p.ocr_coords_text + "\n"
            print(f'  OCR坐标数据:\n{p.ocr_coords_text[:2000]}')
    parsed = nbmc_parser.parse(full_text)
    ...
    for i, result in enumerate(parsed.results[:30], 1):
        ...
        ref = result.reference_text if result.reference_text else (f'{result.reference_low}-{result.reference_high}' if result.reference_low else '')
