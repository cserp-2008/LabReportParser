from sqlalchemy import Column, Integer, String, DateTime, Text, Float, BigInteger, DECIMAL, ForeignKey, Index
...
class Hospital(Base):
    ...
    template_config = Column(Text, comment="JSON: 学习到的解析规则(指标名映射、单位、参考区间格式)")
...
class ReportPage(Base):
    ...
    create_time = Column(DateTime, default=func.now())
    __table_args__ = (
        Index('idx_report_page', 'report_id'),
    )
...
class LabItem(Base):
    ...
    reference_range = Column(String(100), comment="参考范围，如 3.5-9.5 或 男:57-97;女:41-73")
